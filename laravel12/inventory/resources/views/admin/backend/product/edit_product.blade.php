         ...
         <div class="card">
            <div class="row">
               ...
               <div class="col-md-6 mb-3">
                  ...
                  <input type="text" name="code" class=" form-control" placeholder="Enter Code" value="{{ $product->code }}">
               ...
               <div class="col-md-6 mb-3">
                  ...
                  <input type="number" name="stock_alert" class="form-control" value="{{ $product->stock_alert }}"  min="0" required>
                   
               </div>

               <div class="col-md-12">
                  <label class="form-label">Notes: </label>
                  <textarea class="form-control" name="note" rows="3" placeholder="Enter Notes">{{ $product->note }}</textarea>
               </div>
            </div>
         </div>
      </div>
      <div class="col-xl-4">
                ...
                @if(isset($product) && $product->images->count() > 0)
                    @foreach($images as $img)
                        <div class="col-md-3 mb-2">
                            <img src="{{ asset($img->name) }}" alt="Product Image" class="img-thumbnail">

                            <div class="form-check mt-1">
                                <input type="checkbox" name="remove_image[]" value="{{ $img->id }}" id="remove_image_{{ $img->id }}" class="form-check-input">
         ...
         <div>
            <div class="col-md-12 mb-3">
               <h4 class="text-center">Edit Stock : </h4>
            </div>
            <div class="col-md-12 mb-3">
               <div class="form-group w-100">
                  ...
                  <select name="warehouse_id" id="warehouse_id" class="form-control form-select">
                     ...
                     <option value="{{ $item->id }}" {{ $product->warehouse_id == $item->id ? 'selected' : '' }}>{{ $item->name }}</option>
                     @endforeach
                  </select>
            ...
            <div class="col-md-12 mb-3">
               ...
               <input type="number" name="product_qty" class="form-control" value="{{ $product->product_qty }}" min="1" required>
            ...
            <div class="col-md-12">
               <div class="form-group w-100">
                  ...
                  <select name="status" id="status" class="form-control form-select">
                     ...
                     <option value="Received" {{ isset($product->status) && $product->status == 'Received' ? 'selected' : '' }}>Received</option>
                     <option value="Pending" {{ isset($product->status) && $product->status == 'Pending' ? 'selected' : '' }}>Pending</option> 
                  </select>
               </div>
            </div>
         </div>
